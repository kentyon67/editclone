...
**最終ビジョン:** 元動画＋プロンプト（＋参考動画）を送れば、AIが動画を理想の編集済み動画を作成する。Plugin との連携でさらに精度が上がる。pluginは対応ソフト内で、プロンプトや参考動画をもとに、対応ソフト内で可能な編集を自動実行し、ユーザーと対話的に理想の動画が作成できる。
...
- コード作成・編集・リファクタリング
- パッケージ追加（requirements.txt / package.json）
- テスト作成・実行
- Playwright E2Eテスト・スクリーンショット取得
- エラー原因調査・バグ修正・再テスト
- README・ドキュメント更新
- git commit / git push
- アプリ起動・動作確認
- Agent Teams による多角的評価と自動改善
...
- 破壊的な API 変更（既存クライアントが壊れる後方互換性のない変更）

### 実装方針

- 新機能実装前に Style Profile・Project Sync との整合性を確認する
- Web 単体での動作を損なわないか確認する
- 問題・重要な報告・判断が必要な事項はユーザーに報告する
